...
def test_text_box():
    ...
    # user_adress = driver.find_element(By.XPATH, '//*[@id="currentAddress"]').text
    # user_permanent = driver.find_element(By.XPATH, '//*[@id="permanentAddress"]').text
    #
    # print(user_name, user_email, user_adress, user_permanent)

    with allure.step('Check "name" is True'):
        check.equal(user_name, f'Name:{user_test_name}', '"name" is False')

    with allure.step('Check "email" is True'):
        check.equal(user_name, f'Email:{user_test_name}', '"email" is False')
